...
- **Never collapse a sealed result type into a nullable at the caller.** If a function returns a sealed result whose branches carry payload (e.g. `BackendResult.Success(message)` / `Failure(message)`), the caller MUST propagate the sealed type — or destructure both branches — instead of returning `T?` that drops the success payload on the floor. Symptom: asymmetric returns ("`null` on success, error string on failure") forcing the consumer to fabricate placeholder data on the success path. This is exactly the bug that made `query_contacts` return "Found Mumma — mobile +91..." but the brain narrate "I could not find a contact named mumm" — `AgentRuntime.dispatchStep` was returning `String?` and `buildResultMap` hardcoded `"tool $toolName ran"` as the message. The fix was a `StepOutcome` sealed type that carries the real `Success.message` end-to-end. Same rule applies to `RuntimeEvent.Done(summary)`, `BackendResult`, `ToolResult`, and any future result type. Also: prefer sealed-type sentinels over string sentinels for control flow (the dropped `CANCEL_SENTINEL = "__cancelled__"` is the smell).
