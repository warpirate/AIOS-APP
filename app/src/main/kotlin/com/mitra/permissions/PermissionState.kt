...
data class PermissionStatus(val permission: Permission, val granted: Boolean)

data class PermissionSnapshot(val statuses: List<PermissionStatus>) {
...
object Permissions {
    ...
    fun launchGrant(context: Context, permission: Permission) {
        when (permission) {
            Permission.WRITE_SETTINGS -> launchSpecial(
                context,
                Intent(Settings.ACTION_MANAGE_WRITE_SETTINGS)
                    .setData(Uri.parse("package:${context.packageName}")),
            )
            Permission.NOTIFICATION_POLICY -> launchSpecial(
                context,
    ...
    fun runtimePermission(permission: Permission): String? = when (permission) {
