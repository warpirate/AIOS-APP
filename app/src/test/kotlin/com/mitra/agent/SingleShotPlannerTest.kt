...
private class FakeBrain(
    private val turns: List<BrainTurn>,
) {
    fun chatStream(
...
class SingleShotPlannerTest {
    ...
    @Test
    fun `brain emits a tool call - planner returns single-step plan`() =
        runBlocking {
            val brain =
                FakeBrain(listOf(BrainTurn(text = "OK", toolCall = com.mitra.agent.ToolCall("toggle_flashlight", mapOf("on" to true)))))
    ...
    @Test
    fun `brain returns chat-only - planner falls back to parser`() =
        runBlocking {
    ...
    @Test
    fun `brain returns chat-only and parser also has nothing - empty plan`() =
        runBlocking {
    ...
    @Test
    fun `streaming chunks are delivered to onChunk in order`() =
        runBlocking {
            val brain = FakeBrain(listOf(BrainTurn("Hel", null), BrainTurn("Hello", null), BrainTurn("Hello!", null)))
            val chunks = mutableListOf<String>()
            val planner =
                SingleShotPlanner(brainStream = brain::chatStream, parser = parser, sideEffectOf = sideEffectOf, onChunk = { chunks += it })
