...
Updated 2026-06-16. M1 tool surface effectively complete: all V1 device-control + telephony tools shipped (`toggle_flashlight`, `set_alarm`, `start_timer`, `open_url`, `open_app`, `open_settings`, `query_contacts`, `make_call`, `send_sms`, `set_media_volume`, `set_brightness`, `set_dnd`, `set_ringer_mode`, `set_auto_rotate`, `set_screen_timeout`, `set_bluetooth`). M2 trust surface closes today with the AuditHistoryScreen. Focus shifts to P2 brain (cross-turn referent + proactive clarify) before AutomationBackend seam (M5.5) and AccessibilityService work (M6).
